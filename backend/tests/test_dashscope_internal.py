...
class DashScopeInternalTestCase(unittest.TestCase):
    def setUp(self) -> None:
        self.parser = DashScopePayloadParser()

    def test_payload_parser_extracts_and_merges_speaker_segments(self) -> None:
        payload = {
            "output": {
                "results": [
                    {
        ...
        segments = self.parser.extract_segments(payload)
        merged = self.parser.normalize_and_merge_segments(segments)

        self.assertEqual(len(merged), 2)
        self.assertEqual(merged[0].speaker_id, "s1")
        self.assertEqual(merged[0].text, "你好世界")
        self.assertEqual(merged[1].speaker_id, "s2")

    def test_payload_parser_extracts_text_from_nested_payload(self) -> None:
        payload = {
            "output": {
                "results": [
                    {
        ...
        self.assertEqual(self.parser.extract_text(payload), "第一句第二句")

    def test_file_transcriber_raises_timeout_when_task_never_finishes(self) -> None:
