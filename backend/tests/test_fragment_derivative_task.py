...
def _seed_fragment_snapshot(db_session_factory, *, fragment_id: str, text: str) -> str:
    """创建可向量化的 fragment snapshot，供异步回填任务消费。"""
...
@pytest.mark.asyncio
async def test_fragment_derivative_task_backfills_summary_tags_and_vector(
    async_client,
...
@pytest.mark.asyncio
async def test_fragment_derivative_task_uses_fallback_when_llm_fails(
    async_client,
    auth_headers_factory,
    app,
    db_session_factory,
...
@pytest.mark.asyncio
async def test_fragment_derivative_task_logs_vector_failure_without_failing_task(
    async_client,
    auth_headers_factory,
    app,
    db_session_factory,
) -> None:
    """向量写入失败时，异步回填任务仍应成功并保留摘要标签。"""
...
@pytest.mark.asyncio
async def test_fragment_derivative_task_succeeds_without_fragment_projection(
    async_client,
    auth_headers_factory,
    app,
    vector_store,
