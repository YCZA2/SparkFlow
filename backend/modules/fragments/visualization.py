...
def _build_visualization_payload(items: list[tuple[Fragment, list[float]]], used_vector_source: str) -> dict[str, Any]:
    ...
    if cluster_assignments:
        ...
        ordered_clusters = sorted(cluster_members.items(), key=lambda item: (-len(item[1]), fragments_ordered[min(item[1])].id))
        cluster_id_map = {raw_cluster_id: stable_index for stable_index, (raw_cluster_id, _) in enumerate(ordered_clusters, start=1)}
...
async def build_fragment_visualization(
    *,
    ...
    if not vector_documents:
        ...
        return _build_visualization_payload(items=fallback_items, used_vector_source="fallback_text_features")
    ...
    if not valid_items:
        ...
        return _build_visualization_payload(items=fallback_items, used_vector_source="fallback_text_features")
    return _build_visualization_payload(items=valid_items, used_vector_source="vector_store")
