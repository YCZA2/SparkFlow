...
from modules.shared.ports import AudioStorage, MediaAssetStorage
...
class MarkdownExportUseCase:
    ...
    def __init__(self, *, audio_storage: AudioStorage, media_asset_storage: MediaAssetStorage) -> None:
        """装配内容导出所需的本地文件能力。"""
        self.fragment_service = FragmentCommandService(audio_storage=audio_storage)
        self.media_asset_storage = media_asset_storage

    def export_fragment(self, *, db: Session, user_id: str, fragment_id: str) -> tuple[MarkdownExportFile, list[tuple[str, Path]]]:
        """导出单条碎片为 Markdown。"""
        ...
        return MarkdownExportFile(filename=filename, content=render_markdown_document(metadata=metadata, body_markdown=body)), self._asset_files(payload.media_assets)

    def export_script(self, *, db: Session, user_id: str, script_id: str) -> tuple[MarkdownExportFile, list[tuple[str, Path]]]:
        """导出单条脚本为 Markdown。"""
        ...
        return MarkdownExportFile(filename=filename, content=render_markdown_document(metadata=metadata, body_markdown=body)), self._asset_files(media_assets)

    def export_knowledge_doc(self, *, db: Session, user_id: str, doc_id: str) -> tuple[MarkdownExportFile, list[tuple[str, Path]]]:
        """导出单条知识库文档为 Markdown。"""
    ...
    def _asset_files(self, media_assets) -> list[tuple[str, Path]]:
        """将素材资源映射为 zip 内路径和本地路径。"""
        files: list[tuple[str, Path]] = []
        for asset in media_assets:
            resolved = self.media_asset_storage.resolve_path(asset.storage_path)
            files.append((f"assets/{asset.id}-{asset.original_filename}", resolved))
        return files
    ...
    @staticmethod
    def _write_asset_files(*, zip_file: zipfile.ZipFile, asset_files: list[tuple[str, Path]], seen_entries: set[str]) -> None:
        """把素材文件去重写入 zip。"""
        for archive_name, path in asset_files:
            if archive_name in seen_entries or not path.exists():
                continue
