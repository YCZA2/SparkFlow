...
ALLOWED_MEDIA_EXTENSIONS = {
    ...
    "file": {".txt", ".md", ".docx", ".pdf", ".xlsx"},
}

ALLOWED_DOCUMENT_EXTENSIONS = {".txt", ".md", ".docx", ".pdf", ".xlsx"}
...
def normalize_object_key(value: str) -> str:
    ...
    if not normalized:
        raise ValidationError(
            message="对象键不能为空", field_errors={"object_key": "请提供有效对象键"}
        )
    return normalized


def sanitize_filename(value: str, fallback: str) -> str:
    """清洗文件名，避免对象存储和本地路径出现非法字符。"""
...
def validate_media_upload(
    file: UploadFile, media_kind: str, content: bytes
) -> tuple[str, str]:
    """校验素材上传并返回扩展名与 MIME。"""
    ...
    if not content:
        raise ValidationError(
            message="上传文件为空", field_errors={"file": "请选择有效文件"}
        )
    mime_type = (
        file.content_type
...
def validate_document_upload(file: UploadFile, content: bytes) -> tuple[str, str]:
    """校验上传文档并返回扩展名与 MIME。"""
    ext = Path(file.filename or "").suffix.lower()
    if ext not in ALLOWED_DOCUMENT_EXTENSIONS:
        raise ValidationError(
            message="不支持的文档格式",
            field_errors={"file": "支持 .txt、.md、.docx、.pdf、.xlsx"},
        )
    if not content:
        raise ValidationError(
            message="上传文件为空", field_errors={"file": "请选择有效文件"}
    ...
    if len(content) > MAX_DOCUMENT_FILE_SIZE:
        raise ValidationError(
            message="文档文件过大",
            field_errors={"file": "文档文件大小不能超过 20MB"},
        )
    mime_type = (
        file.content_type
        or mimetypes.guess_type(file.filename or "")[0]
...
def build_document_object_key(*, user_id: str, fragment_id: str, filename: str) -> str:
    """构造文档对象 key。"""
    return normalize_object_key(f"documents/{user_id}/{fragment_id}/{filename}")
...
def build_imported_audio_object_key(
    *, user_id: str, fragment_id: str, platform: str, filename: str
) -> str:
    """构造外链导入音频对象 key。"""
    return normalize_object_key(
        f"audio/imported/{user_id}/{fragment_id}/{platform}/{filename}"
    )
...
class LocalFileStorage(FileStorage):
    ...
    async def save_local_file(
        ...
        *,
        ...
        if not source.exists():
            raise ValidationError(
                message="源文件不存在", field_errors={"file": "临时文件不存在"}
            )
        destination = self._destination(object_key)
    ...
    def create_download_url(self, stored_file: StoredFile) -> FileAccess:
        """为本地对象构造后端静态访问地址。"""
        return FileAccess(
            url=f"/uploads/{normalize_object_key(stored_file.object_key)}",
            expires_at=None,
        )
    ...
    def materialize(self, stored_file: StoredFile) -> MaterializedFile:
        """直接返回本地对象路径。"""
        local_path = (
            self.upload_dir / normalize_object_key(stored_file.object_key)
        ).resolve()
...
class OssFileStorage(FileStorage):
        ...
        *,
        ...
        try:
            ...
            raise ServiceUnavailableError(
                message=f"OSS SDK 不可用: {exc}", service_name="oss"
            ) from exc
        ...
        self.public_base_url = (
            public_base_url.strip().rstrip("/") if public_base_url else None
        )
    ...
    async def save_upload(
        ...
        *,
        ...
        self.bucket.put_object(
            normalize_object_key(object_key), content, headers=headers
        )
        return self._build_stored_file(
            object_key=object_key,
            original_filename=original_filename,
    ...
    def create_download_url(self, stored_file: StoredFile) -> FileAccess:
        ...
        expires_at = datetime.now(timezone.utc) + timedelta(
            seconds=self.url_expire_seconds
        )
    ...
    def materialize(self, stored_file: StoredFile) -> MaterializedFile:
        ...
        temp_file = tempfile.NamedTemporaryFile(
            prefix="sparkflow-oss-", suffix=suffix, delete=False
        )
        temp_path = Path(temp_file.name)
        temp_file.close()
        self.bucket.get_object_to_file(object_key, str(temp_path))
        return MaterializedFile(
            local_path=temp_path, cleanup=lambda: temp_path.unlink(missing_ok=True)
        )
