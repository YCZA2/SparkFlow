...
def parse_uploaded_text(*, file_content: bytes, filename: str) -> str:
    ...
    raise ValidationError(message="文件格式不支持", field_errors={"file": "仅支持 .txt、.docx、.pdf、.xlsx 格式"})
...
def _parse_docx(file_content: bytes) -> str:
    ...
    try:
        ...
        raise ValidationError(message="Word 文件解析失败", field_errors={"file": "请确认上传的是有效的 .docx 文件"}) from exc
...
def _extract_pdf_text_fallback(file_content: bytes) -> str:
    ...
    if not blocks:
        raise ValidationError(message="PDF 文件解析失败", field_errors={"file": "请确认 PDF 中包含可提取文本"})
    texts: list[str] = []
    for block in blocks:
        for raw in re.findall(r"\((.*?)\)", block, flags=re.S):
            text = raw.replace(r"\(", "(").replace(r"\)", ")").replace(r"\n", "\n")
            cleaned = text.encode("latin-1", errors="ignore").decode("utf-8", errors="ignore").strip()
            if cleaned:
...
def _extract_xlsx_text_fallback(file_content: bytes) -> str:
    ...
    try:
        ...
        raise ValidationError(message="Excel 文件解析失败", field_errors={"file": "请确认上传的是有效的 .xlsx 文件"}) from exc
