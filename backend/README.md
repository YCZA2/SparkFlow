...
- fragment 衍生回填已从摘要 / 标签扩展为碎片语义层：服务端生成 `summary / system_tags / system_purpose / vector`；用户侧的 `user_tags / user_purpose / dismissed_system_tags` 由移动端本地真值和备份快照同步。
...
- `modules/scripts/`: 口播稿生成、脚本生成任务定义、上下文构建、结果回流、列表、详情、更新、删除、每日推盘；普通脚本生成按 fragment 用途区分内容素材、风格参考、方法论和补充背景，其中 daily push 现在通过备份快照 reader 聚合 fragment 真值。
...
- transcript 或 fragment backup snapshot 落库后会最佳努力创建内部 `fragment_derivative_backfill` task，异步执行摘要、系统标签、主要用途和向量回填；该回填现在支持只有 `local_fragment_id`、没有 `Fragment` projection 的 local-first 路径，失败也不会回滚已成功的 ingest
