...
- fragment 衍生回填已从摘要 / 标签扩展为碎片语义层：服务端生成 `summary / system_tags / system_purpose / vector`；标签简化为备忘录式体验，只保留 `user_tags`，不再向用户暴露 `dismissed_system_tags` 交互；主要用途由 LLM 自动判断，不再保留 `user_purpose` 用户修正字段。
- `media_ingestion` 已调整为 transcript-first：主任务在转写落库后即可成功，`summary` / `tags` / vector 改为异步衍生回填，不再阻塞上传和抖音导入主链路。
- Chroma 查询适配层已支持当前 `list_collections()` 返回字符串列表的行为，`/api/fragments/similar`、向量文档列表和 namespace 统计不会再因版本差异误判为空。
...
- `script` 继续保留独立后端业务表与路由层；local-first 共享的是 backup/recovery 基础设施，不是把 fragment / script 合并为同一业务实体；后端 `scripts` 表当前只保存生成初稿和任务完成后的详情读取，不再承担移动端编辑真值语义。
- 共享文档解析器 `modules/shared/content/document_parsers.py` 现在只依赖 `python-docx`、`pypdf`、`openpyxl` 作为正式解析实现；PDF/XLSX 已移除手写 fallback，损坏文件统一返回稳定的 `ValidationError`。
...
- script 生成仍由后端 task runtime 驱动，但成功后客户端会把脚本详情立即落本地，后续编辑与拍摄状态同步走备份链路；列表与详情主读取都以客户端本地真值为准
...
当前脚本生成已经升级为“三层写作上下文 + 写作风格 + 主题 + SOP 大纲”链路，不再使用 `mode_a / mode_b` 两套独立工作流配置。

当前生成链路依赖：

- `LLM_PROVIDER` 对应的文本生成能力，用于碎片方法论离线提炼、大纲和草稿生成
- `Embedding + VectorStore`，用于检索相关碎片
- `POST /api/scripts/generation` 的输入收敛为必填 `topic` + 可选 `fragment_ids`
- `UserWritingStyle` 提供用户独立维护的写作风格描述，供生成时引用
...
如果你想验证“文档导入碎片 + 语义回填 + 脚本生成”整条链路，可以运行：
...
- 调用 `POST /api/imports/document` 将文档导入为碎片
- 轮询文档导入任务直到异步处理完成
- 验证碎片摘要、标签和向量回填结果
- 创建 1-2 条手动碎片并触发 `POST /api/scripts/generation`
- 轮询 `GET /api/tasks/{task_id}` 直到终态
- 成功后读取 `GET /api/scripts/{script_id}` 并输出脚本摘要
- 传入 `--cleanup` 时自动删除本次联调创建的碎片和脚本

知识库后端 API 仍保留兼容，但移动端已移除知识库入口；文档统一走碎片导入链路。
...
6. `modules/shared` + `services`
   ...
   - `backend/modules/shared/infrastructure/infrastructure.py` 仅作为聚合导出入口，真实实现拆到 `storage.py`、`vector_store.py`、`providers.py`。
...
- `modules/fragments/`: 当前只保留标签、相似检索、可视化、碎片语义回填和 fragment snapshot 详情 / 导出组装能力。
...
- `modules/scripts/`: 口播稿生成、脚本生成任务定义、上下文构建、结果回流、列表、详情、更新、删除、每日推盘；普通脚本生成按 fragment 用途区分内容素材、风格参考、方法论和补充背景，并引用用户独立维护的 `UserWritingStyle` 作为写作风格输入；daily push 现在通过备份快照 reader 聚合 fragment 真值。
...
- `POST /api/knowledge/search` 对外仍返回文档级结果，但内部已经改为 chunk 召回后聚合，并会附带 `matched_chunks` 供后续 citation 能力使用。
- `scripts` 的 RAG 生成现在从 `UserWritingStyle` 读取固定写作风格描述，从碎片向量库召回相关素材；知识库独立检索已降级为兼容能力，不再作为生成主链路输入。
...
- `POST /api/knowledge/upload` 仍保留为后端兼容能力，但移动端不再调用
...
- transcript 或 fragment backup snapshot 落库后会最佳努力创建内部 `fragment_derivative_backfill` task，异步执行摘要、系统标签、用途判断（仅内部使用）和向量回填；该回填现在支持只有 `local_fragment_id`、没有 `Fragment` projection 的 local-first 路径，失败也不会回滚已成功的 ingest
- SparkFlow 后端先收集预置稳定内核、已缓存方法论、用户写作风格、相关素材和可选碎片背景，再生成大纲与草稿
...
- 当前后端不提供 `GET /api/transcriptions/{fragment_id}`、`GET /api/fragments` 与 `GET /api/fragments/{fragment_id}` 这三条读取接口；状态查询统一走 `task_id -> /api/tasks/{task_id}`，fragment 主读取统一走移动端本地真值
