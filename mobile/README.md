...
- **Android 支持**：`app.json` 补齐 Android 权限、自适应图标配置，`dev-mobile.sh` 新增 `android` 子命令，支持 Android 真机与模拟器联调。
- **认证改造**：登录页已切换为邮箱 + 密码模式，移除手机验证码入口；注册/登录成功后继续沿用设备会话机制。
- **备忘录风格改版**：首页、文件夹页、成稿列表调整为极简备忘录风格，碎片卡片改为纯文本条目，快速操作收口到底部悬浮按钮。
- **编辑器优化**：新增 HTML 包装/解包层解决 `<p>` 空节点问题，转写后正文首尾不再出现空白行；快照保存链路简化，显式保存优先读取最新缓存。
- **转写与转录**：音频转写保留句级说话人切片并合并重叠片段；转录失败时列表页显示错误提示并支持下拉重试。
- **导航改进**：实现单航班导航推送，避免重复压栈导致的返回路径混乱；成稿编辑页保存按钮交互与碎片页对齐。
- `fragments / folders` 的 local-first phase 1 已经落地完成：列表、详情、编辑、删除都以本地 SQLite + 文件系统为真值，远端只负责备份与恢复。
- 移动端正式入口已经切到”登录后工作区”：未登录只显示邮箱密码登录/注册页；登录成功后才挂载本地 SQLite、正文文件和备份队列。
...
- 当前不再自动使用测试用户进入主流程；正式登录默认走邮箱 + 密码，`/api/auth/token` 仅用于本地开发联调
