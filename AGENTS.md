...
- SparkFlow should not ask users to maintain an independent knowledge base in the mobile product flow. User-facing imports should become fragments, and fragment semantics should live on fragment snapshots as `system_tags / user_tags / dismissed_system_tags` and `system_purpose / user_purpose`; user corrections always win over system suggestions.
- Fragment purpose is a discrete enum without confidence scores: `content_material`, `style_reference`, `methodology`, `case_study`, `product_info`, `other`. Script generation must treat style-reference fragments as guidance for how to write, not as factual content to copy.
- Mobile `fragment / folder / script` async reads now use React Query as the primary read layer. Query keys must stay bound to `user_id + session_version + workspace_epoch`, query functions should read local SQLite / files directly, and Zustand should be limited to UI / session runtime state rather than entity list/detail caches
...
- New mobile UI should prefer NativeWind `className` utilities and Tailwind tokens from `mobile/tailwind.config.js`. Keep `StyleSheet.create` for animation-heavy, computed, or third-party-constrained styles, treat `mobile/theme/tokens.ts` as a compatibility mirror rather than the source for new design tokens, and do not reintroduce `components/Themed.tsx` / `constants/Colors.ts`.
- Shared document parsing now treats `python-docx`, `pypdf`, and `openpyxl` as required runtime dependencies. Do not add new PDF/XLSX fallback parsers; map invalid files to stable `ValidationError` responses instead.
