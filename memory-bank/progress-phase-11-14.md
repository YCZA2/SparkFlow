...
| 11.3 文件上传解析 | 已完成 | 支持 `.txt` / `.docx` / `.pdf` / `.xlsx` |
...
- JSON 创建知识库文档。
- 表单上传 `.txt` / `.docx` / `.pdf` / `.xlsx` 文件并解析内容。
- 知识库正文统一落到 `body_markdown`，并兼容下游纯文本提取。
- 按 `doc_type` 过滤与分页查询。
- 基于向量库进行知识库搜索，内部已切到 chunk 召回后聚合。
- 删除或更新时同步删除并重建对应知识索引。
- `reference_script` 保留异步处理链路，风格描述与分块索引统一回写到知识库记录。
- 知识库已不再只作为“文档库”理解，而是开始承担脚本生成长期背景的来源之一。
...
| 12.4 三层写作上下文增强 | 部分完成 | 稳定内核已切系统预置，方法论改后台缓存，素材召回已落地；个性化画像待后续迭代 |
| 12.5 碎片向量可视化 | 已完成 | `GET /api/fragments/visualization` |
| 12.6 知识库向量化增强 | 已完成 | 已落地 chunk 索引、文档聚合搜索与生成联动 |

### 当前实现位置

- Vector adapter: [`backend/modules/shared/infrastructure/vector_store.py`](/Users/hujiahui/Desktop/VibeCoding/SparkFlow/backend/modules/shared/infrastructure/vector_store.py)
- Container entry: [`backend/modules/shared/infrastructure/container.py`](/Users/hujiahui/Desktop/VibeCoding/SparkFlow/backend/modules/shared/infrastructure/container.py)
...
- `body_markdown` 已能进入脚本上下文、daily push 与知识库消费链路；移动端文本编辑链路已采用**缓存优先 + 乐观更新**架构。
- 可按语义查询相似碎片，并回表补齐摘要、标签、来源和时间。
- 灵感云图可批量读取用户向量，做 PCA / 聚类 / fallback projection。
- 知识库文档创建时会同步写入 `knowledge_{user_id}`，当前按 chunk 级索引存储。
- 脚本生成侧已经从“few-shot/reference-script 检索”升级为“三层写作上下文”：
- 稳定内核层由历史碎片和长期资料自动聚合，不使用 AI 历史脚本反哺人格层。
- 方法论层由碎片提炼、上传资料和预置模板构成。
- 相关素材层继续动态召回历史脚本、碎片和知识文档。

### 当前缺口

- 三层写作上下文还需要继续打磨命中质量、相关性排序和去噪策略。
- 知识库侧还没有混合检索、重排序、多模态 OCR 等进一步增强策略。
...
- 每日推盘正文生成当前仍由后端直连 `llm_provider.generate(...)` 完成，不依赖 Dify workflow。
