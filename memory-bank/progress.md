...
| 阶段 11 | 知识库基础（后端已完成，前端跳过） | 已完成 | 80% | [查看详情](progress-phase-11-14.md) |
| 阶段 12 | 向量数据库集成 | 进行中 | 55% | [查看详情](progress-phase-11-14.md) |
| 阶段 13 | 每日灵感推盘 | 待开始 | 0% | [查看详情](progress-phase-11-14.md) |
| 阶段 14 | 收尾与全流程验证 | 待开始 | 0% | [查看详情](progress-phase-11-14.md) |
...
- **阶段 12.2**: 碎片语义相似度查询 ✅（2026-03-06）
  ...
  - 新增 `query_similar_fragments()`，支持 `top_k` 与 `exclude_ids`
  - 查询结果会回表补充摘要、标签、来源、时间等碎片字段
  - 缺失数据库记录的向量结果会自动过滤
  - 补充后端测试，覆盖 API 返回与缺失记录过滤场景

- **阶段 12.1**: 碎片自动向量化 ✅（2026-03-06）
  - 新增 `services/vector_service.py` 封装碎片向量写入
  - 转写成功后自动生成 embedding 并写入 ChromaDB
  - Collection 按用户隔离：`fragments_{user_id}`
  - 向量化失败时仅记录日志，不影响转写主流程
  - 补充后端测试，覆盖成功写入与失败降级场景

- **阶段 11**: 知识库后端 API 完成，前端入口跳过 ⏭️（2026-03-06）
  - ✅ 11.1-11.3 后端 API 完成（上传、列表、文件解析）
  - ⏭️ 11.4 前端知识库管理入口 **跳过**（产品思路待明确后实施）
  - **决策**: 直接进入阶段 12 向量数据库集成
...
1. **阶段 12**: 向量数据库集成（调整方向）
   - 碎片转写成功后自动写入向量库
   - 实现碎片语义相似度查询（用于每日推盘关联检查）
   - Mode B 生成时检索用户历史碎片作为风格参考
   - **碎片向量可视化**（灵感云图，MVP 轻量版）
   - 知识库文档向量化（可选）
...
| 日期 | 问题 | 决策 |
|------|------|------|
| 2026-03-06 | 阶段 12.4 实施路径 | **允许先跳过 12.3**，12.4 改为轻量 MVP：后端读取现有向量并生成可视化坐标/聚类，前端先做轻量云图，不引入重型 3D 方案 |
| 2026-03-06 | 知识库前端 | **跳过** 阶段 11.4 前端入口，直接进入阶段 12 |
| 2026-03-06 | 阶段 12 方向 | 聚焦**碎片语义检索**，而非知识库文档向量化 |
