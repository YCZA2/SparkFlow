...
2. **按钮交互**
   - 点击大圆形按钮，按钮文字从"开始录音"变为"停止录音"
   - 按钮颜色保持红色（略微加深）
   - 上方状态文字从"点击开始录音"变为"正在录音…"
   - 底部提示文字变为"点击按钮结束录音"
...
1. **首次录音 - 权限请求**
   - 点击"开始录音"按钮
   - 应弹出系统权限对话框："SparkFlow-mobile 想要访问您的麦克风"
   - 点击"允许"

2. **录音状态显示**
   - 按钮变为"停止录音"
   - 上方文字变为"正在录音… 00:00"并开始计时
   - 底部提示变为"再次点击结束录音"
   - 按钮阴影增强（视觉反馈）
...
4. **停止录音**
   - 点击"停止录音"按钮
   - 录音完成，显示"录音完成 (00:XX)"
   - 中间区域显示录音文件路径（file:///var/mobile/.../recording.m4a）
   - 底部提示变为"录音已保存，等待上传"
...
**碎片库列表页面 (`app/(tabs)/fragments.tsx`)**
- 使用 FlatList 渲染碎片列表
