...
- 使用个人开发者证书（免费）或团队证书
- 连接 iPhone 设备，信任开发者证书
- 在 Expo 中配置 `eas build` 或直接使用 `expo run:ios --device`
...
| 离线支持 | 录音后无网络时暂存本地，碎片库支持离线查看 | 需实现本地队列 + 同步机制 |
...
### 步骤 2.11：音频存储策略（MVP简化）

**决策：** MVP 阶段直接使用 `.m4a` 格式，不进行转码。

- **理由：** Whisper API 已支持 `.m4a` 格式，转码增加复杂度和依赖（需要 pydub + ffmpeg）
- **存储路径：** `uploads/{user_id}/{uuid}.m4a`
- **未来预留：** 如需转码，可在 `backend/services/audio_converter.py` 中实现，通过配置开关控制

**验证测试：** 上传一个 `.m4a` 文件，Whisper 直接转写，准确率和响应时间可接受。
...
**网络状态处理：**
- 有网络：直接上传，显示"上传成功"提示
- 无网络：将录音文件存入本地待上传队列（SQLite），显示"已保存，将在网络恢复后自动上传"
...
2. 飞行模式：点击录音 → 停止 → 显示"已保存到本地"，碎片库显示"待同步"状态
3. 恢复网络：自动上传队列中的录音，同步完成后状态更新为"已转写"
...
修改 `POST /api/scripts/generate` 中 Mode B 的逻辑：在调用 LLM 前，使用碎片内容作为查询文本，调用 `query_similar` 检索该用户知识库中最相关的文本片段（最多3段）。将检索到的参考文本作为 LLM system prompt 的补充上下文（如"以下是用户过往的写作风格参考：..."）。这使 Mode B 真正具备"学习用户个人表达"的能力。
...
在 `backend/services/scheduler.py` 中，编写函数 `daily_aggregate()`。逻辑：查询每个用户昨天（过去 24 小时内）创建的碎片笔记。如果某用户的碎片数量 >= 3 条，则将这些碎片的 `transcript` 合并，使用 Mode A 的 Prompt 调用 LLM 生成一篇口播稿。将生成的口播稿写入 `scripts` 表，`is_daily_push` 标记为 `true`。
