...
### 步骤 12.4：碎片向量可视化（MVP 轻量版）

**目标**：复用用户碎片已落库的向量数据，先打通"向量读取 → 坐标生成 → 聚类分组 → 轻量云图展示"链路。第一版优先保证稳定和低复杂度，不直接引入重型 3D 技术栈。

**关键决策**：
- 允许先跳过 12.3，12.4 不依赖 Mode B 的风格参考检索
- 不重复生成 embedding，直接读取 ChromaDB 中已存在的向量
- 聚类基于**原始 embedding**，展示坐标基于降维结果，两者职责分离
- 首版不引入 LLM 实时命名类别，不把额外模型调用放进主请求链路
- 前端首版先做 2D/2.5D 灵感云图，保留 `z` 坐标为后续 3D 升级做准备

**技术方案**：

| 层级 | 技术选型 | 说明 |
|------|----------|------|
| 向量来源 | **ChromaDB 现有 embedding** | 直接复用阶段 12.1 已写入的碎片向量 |
| 降维算法 | **PCA (3D)** | 成本低、结果稳定、便于测试与解释 |
| 聚类算法 | **K-Means** | 仅在碎片数量达到阈值时启用 |
| 后端 API | `GET /api/fragments/visualization` | 当前用户上下文读取，不通过请求体传 `user_id` |
| 前端渲染 | **轻量 2D/2.5D 云图** | 优先基于 React Native 轻量渲染，避免直接引入 Three.js |
| 类别命名 | **tags + summary 关键词统计** | 使用现有碎片数据生成类别标签 |

**实施步骤**：

1. **补充向量读取能力**
   - 在 `backend/services/base/base_vector_db.py` 增加批量读取命名空间文档的方法
   - 在 `backend/services/chroma_vector_db.py` 中封装对 collection `get()` 的读取
   - 返回字段至少包含：`id`、`embedding`、`document`、`metadata`

2. **实现后端聚合服务**
   - 新增可视化服务函数，例如 `backend/services/vector_visualization_service.py`
   - 从向量库读取当前用户全部碎片向量
   - 回表 SQLite，过滤数据库已删除的碎片并补齐 `summary`、`tags`、`created_at`

3. **实现降维与聚类**
   - 使用 PCA 将 embedding 从 1536 维降到 3 维
   - 使用 K-Means 在原始 embedding 上聚类，而不是在 3D 坐标上聚类
   - 聚类数自动估算：`cluster_count = min(6, max(2, round(sqrt(n / 2))))`

4. **实现降级逻辑**
   - 碎片数 `< 5`：只返回点位，不聚类
   - 碎片数 `5-7`：返回点位，聚类可选
   - 聚类异常时：整体退化为仅坐标展示
   - 用户无向量或集合不存在时：返回空结果而不是报错

5. **生成类别标签**
   - 优先统计簇内 tags 高频词
   - tags 不足时，从 summary 中提取高频关键词
   - 取前 1-3 个关键词作为类别名

6. **前端首版展示**
   - 新增"灵感云图"页面入口
   - 使用 `x/y` 作为平面布局，`z` 映射为大小、阴影或透明度
   - 点击点位查看碎片详情
   - 点击类别标签筛选该类碎片
   - 支持从当前类别继续选中碎片生成口播稿
...
GET /api/fragments/visualization
    ↓
读取当前用户全部碎片向量 (ChromaDB)
    ↓
回表 SQLite 过滤无效碎片并补齐业务字段
    ↓
PCA 生成 3D 坐标
    ↓
K-Means 基于原始 embedding 聚类（满足阈值时）
    ↓
基于 tags/summary 统计类别关键词
    ↓
返回 points + clusters + stats + meta
    ↓
前端渲染轻量灵感云图
```
...
- UMAP / t-SNE / HDBSCAN
- AI 实时命名类别
- `GET /api/fragments/cluster/{cluster_id}` 独立接口
- 3D 球体包裹、力导向连线、复杂镜头控制
- 双击聚焦、悬停高亮等重交互

**验证测试**：
1. 用户至少有 8 条碎片时，API 返回非空 `points` 与 `clusters`
2. 用户碎片少于 5 条时，接口正常降级，不因聚类条件不足而报错
3. 向量库存在但数据库不存在的碎片会被自动过滤
4. 类别标签可由 tags/summary 统计生成，不依赖 LLM
5. 前端可基于单次接口返回完成点位点击与类别筛选
