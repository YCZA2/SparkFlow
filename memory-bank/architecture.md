...
- 后端现已补齐一套阿里云单机部署口径：Ubuntu ECS 上通过 `systemd + nginx` 启动 FastAPI、Celery worker 与 Celery beat，同机承载 PostgreSQL、RabbitMQ、Chroma 和本地 uploads，并把 `/api/*`、`/uploads/*` 挂到同域名下；发布默认走 `ssh aliyun + rsync + 外置 backend.env`。
