...
- `expo-document-picker`: 当前主要用于正文图片与文件选择；文档导入已统一走碎片链路，不再维护独立知识库移动端入口。
...
- 知识库 namespace：`knowledge_{user_id}`（保留兼容，移动端不再调用）
- 脚本生成上下文策略：当前已升级为”三层写作上下文 + 写作风格”
- `稳定内核层`：由历史碎片和长期资料后台聚合为用户级画像，不使用 AI 历史脚本反哺
- `方法论层`：由碎片提炼和预置模板共同组成
- `写作风格层`：用户独立维护的固定风格描述文本，描述表达习惯
- `相关素材层`：动态召回与主题相关的历史脚本和碎片语义资产
...
- 当前项目采用**local-first 真值 + 后端备份/恢复**：fragments / folders / scripts 的内容真值在移动端 SQLite + 文件系统，后端 PostgreSQL 负责账户、任务和备份快照，不再作为这些内容的主读写源。
- Expo Dev Client 已接入，因此原生能力变更需要区分”只改 JS/TS”与”需要重建”两类流程。
- 移动端首页已偏向”碎片管理与创作入口”，不是最初的单一录音主页。
- 知识库后端仍保留为兼容能力，但移动端已移除知识库入口；所有内容统一为碎片，文档导入直接走碎片链路。
